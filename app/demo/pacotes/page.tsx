  ...
  return (
    ...
    >
      ...
      <div className="mt-2 flex items-baseline justify-between gap-2 text-[12px]">
        ...
        <span className="text-right text-[#737373]">{pacote.validadeCard}</span>
      </div>

      {pacote.notaCard && (
        <p className="mt-2.5 border-t border-[#ECECEC] pt-2.5 text-[12px] text-[#737373]">
          {pacote.notaCard}
        </p>
      )}
    </button>
  ...
  return (
    <div>
      <button
        type="button"
        onClick={onBack}
        className="inline-flex cursor-pointer items-center gap-1.5 text-sm text-[#737373] transition hover:text-[#0F0F0F] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#FF6F61] focus-visible:ring-offset-2 focus-visible:ring-offset-[#FBF6F3]"
      >
        <ArrowLeft className="size-4" /> voltar
      </button>

      <div className="mt-4 rounded-3xl border border-[#ECECEC] bg-white p-5">
        <p className="text-base font-semibold text-[#0F0F0F]">{pacote.nome}</p>
        <p className="mt-0.5 text-sm text-[#404040]">{pacote.procedimento}</p>
        ...
        <div className="mt-2 flex items-baseline justify-between gap-2 text-[12px]">
          ...
          <span className="text-right text-[#737373]">
            {pacote.validadeDetalhe}
          </span>
        </div>
      </div>
    ...
    >
      {pacoteAberto ? (
        ...
        <div key="lista" className="bubble-enter space-y-4">
          {PACOTES.map((p) => (
            <PacoteCard key={p.slug} pacote={p} onOpen={setView} />
          ))}
        </div>
      )}
      ...
      {toastVisible && (
        <div
          role="status"
          aria-live="polite"
          className="bubble-enter pointer-events-none fixed inset-x-0 bottom-6 z-50 flex justify-center px-5"
        >
          <span className="rounded-full bg-[#0F0F0F] px-4 py-2.5 text-center text-[13px] text-white shadow-lg">
            Mensagem entraria na fila da Bella (demonstração)
          </span>
        </div>
      )}
    </DemoShell>
